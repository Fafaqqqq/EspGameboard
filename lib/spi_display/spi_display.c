...
static spi_data_t spi_init_data_buf[SPI_INIT_DATA_CNT] = {
  {
...
{
  uint8_t data[2];
  if((x<0)||(y<0)||(x>=DISPLAY_SIZE_X)||(y>=DISPLAY_SIZE_Y)) return;
  data[0] = color>>8;
  data[1] = color & 0xFF;
...
void spi_display_init(spi_device_handle_t spi, uint16_t w_size, uint16_t h_size)
{
  ...
  spi_display_reset();
  spi_display_cmd_send(spi, SPI_DEVICE_RESET);

  vTaskDelay(1000 / portTICK_PERIOD_MS);

  for (int i = 0; i < SPI_INIT_DATA_CNT; ++i)
  {
    spi_display_cmd_send(spi, spi_init_data_buf[i].cmd);
    spi_display_data_send(spi, spi_init_data_buf[i].data, spi_init_data_buf[i].lenght);
  }
